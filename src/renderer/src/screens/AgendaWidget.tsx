...
export default function AgendaWidget() {
  ...
  return (
    ...
    >
      <p className="font-data text-libelle uppercase tracking-[0.1em] text-muted">Aujourd'hui</p>
      {items.length === 0 ? (
        <p className="text-secondaire text-muted">Rien de planifié aujourd'hui.</p>
      ) : (
        ...
        <ul className="flex flex-col gap-2 overflow-y-auto [-webkit-app-region:no-drag]">
          {items.map((item) => (
            <li key={item.id} className="flex items-baseline gap-3">
              ...
              <span
                className={`min-w-0 flex-1 truncate text-secondaire ${item.done ? 'text-muted line-through' : 'text-champagne'}`}
              >
                {item.name}
              </span>
            </li>
